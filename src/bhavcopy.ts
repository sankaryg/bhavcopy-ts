...
export default class BhavCopy {
   ...
   * @return newNum
   */
  __appendZeroToDay(day: string | number) {
    let newNum = ''
    if (day < 10) {
      newNum = '0' + parseInt(day as string, 10)
    } else {
      newNum = '' + day
    }
    return newNum
   ...
   */
  __generateFileNames(criteria: { month: any; year: any; day: any }) {
    const { month, year, day } = criteria
    const baseUrl = 'https://archives.nseindia.com/content/historical/EQUITIES/'
   ...
   * @return Promise
   */
  __getBhavCopyFromNSE(fileName: string) {
    if (fileName) {
        ...
        return this.__callNSEforFile(fileName)
          .then((streamObj: any) => {
            if (streamObj && typeof streamObj === 'object' && Object.keys(streamObj).length) {
              streamObj.on('response', (response: { statusCode: number }) => {
   ...
   * @return Promise
   */
  __callNSEforFile(reqUrl: string) {
    const reqOpts = {
      url: reqUrl,
      method: 'GET',
   ...
   * @return Promise
   */
  download(reqObject: { month: any; year: any; day: any }) {
    return new Promise((resolve, reject) => {
      ...
      if (
        ...
        year === '' ||
        year === null ||
        this.__yearsCode().indexOf(parseInt(year, 10)) === -1
      ) {
        return reject({
